...
from tkinter import Scrollbar, scrolledtext, filedialog
import time
...
def buscar_proceso():
    ...
    for proc in psutil.process_iter(attrs=['PID', 'NOMBRE', 'USUARIO', 'CPU %', 'MEMORIA %', 'ESTADO']):
        try:
...
tree = ttk.Treeview(admin, columns=("PID", "Nombre", "CPU (%)", "Memoria (MB)", "Estado"), show="headings")
for col in ("PID", "Nombre", "CPU (%)", "Memoria (MB)", "Estado"):
